...
export default function ButtonDeleteTurista({ onDeleted }) {
    ...
    const handleDelete = async () => {
        ...
        try {
            const response = await fetch("http://localhost:8000/administrador/eliminar-perfil", {
                method: "DELETE",
                headers: {
            ...
            if (response.ok) {
                ...
                localStorage.removeItem("token_admin"); // Limpiar token
                if (onDeleted) onDeleted(); // callback opcional
                navigate("/administrador"); // redirigir al login
            } else {
